...
test('테이블 CSV는 현재 화면 컬럼 기준으로 내보낸다', async ({ page }) => {
  ...
  expect(csv).toContain('"2026-01-01",300000');
  expect(csv).toContain('"2026-02-01",700000');
...
test('분석 내보내기는 전체실입금내역 시트와 월별 시트를 만든다', async ({ page }) => {
  ...
  expect(workbook).toContain('<Data ss:Type="Number">1000000</Data>');
...
test('분석과 고객관리에서는 월 필터를 숨기고 고객관리 라벨을 명확히 표시한다', async ({ page }) => {
  ...
  await expect(page.getByText('총 대관횟수', { exact: true })).toBeVisible();
